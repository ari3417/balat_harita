...
# ==========================================
# 2. GITHUB KLASÖR YOLLARI
# (GitHub'daki klasör isimlerinin bu şekilde olduğuna emin ol)
# ==========================================
...
# ==========================================
# 3. VERİ İŞLEME VE HARİTA ÜRETİMİ (ÖN BELLEKLİ)
# ==========================================
@st.cache_data
def haritayi_hazirla():
    ...
    most_dominant_color = palet_25_colors[np.argmax(counts)]
                                ...
                                for dosya_adi in os.listdir(KLASOR_SEMBOLIK_ABSTRACT):
                                    ...
                                    if temiz_isim in e_temiz_isim or e_temiz_isim in temiz_isim:
                                        ...
                                        if img is not None:
                                            ...
                                            break 
                            manuel_pin_data.append({"isim": isim, "lat": lat, "lon": lon, "b64": bulunan_b64})
        except Exception as e:
            print("KML okuma hatası:", e)
        ...
        if os.path.exists(folder_path):
            for img_name in os.listdir(folder_path):
                if img_name.endswith(('.png', '.jpg', '.jpeg')):
                    b64 = get_landmark_base64(os.path.join(folder_path, img_name))
    ...
    min_lat = binalar_gdf.geometry.centroid.y.min()
    max_lat = binalar_gdf.geometry.centroid.y.max()
    min_lon = binalar_gdf.geometry.centroid.x.min()
    max_lon = binalar_gdf.geometry.centroid.x.max()

    # ==========================================
    # 4. HTML ŞABLONU (Düz Metin - f-string DEĞİL)
    # ==========================================
...
<head>
    ...
    <style>
        ...
        .dominant-circle { width: 32px; height: 32px; border-radius: 50%; background: __MOST_DOMINANT_COLOR__; box-shadow: 0 2px 5px rgba(0,0,0,0.2); flex-shrink: 0; }
...
<body>
    ...
    <div id="sidebar-wrapper">
        ...
        <div id="sidebar">
            ...
            <div class="sidebar-content">
                ...
                <div class="dominant-color-container" onclick="filterColor('__MOST_DOMINANT_COLOR__')">
    ...
    <script>
        const minLat = __MIN_LAT__ - 0.002;
        const maxLat = __MAX_LAT__ + 0.002;
        const minLon = __MIN_LON__ - 0.002;
        const maxLon = __MAX_LON__ + 0.002;
        ...
        const balatSiniri = __BALAT_GEOJSON__;
        const topColors = __PALET_JSON__;
        const yollarData = __YOLLAR_GEOJSON__;
        const specialLandmarksGercek = __LANDMARKS_GERCEK_JSON__;
        const specialLandmarksAbstract = __LANDMARKS_ABSTRACT_JSON__;
        const manuelPinsData = __MANUEL_PIN_JSON__;
    ...
    html_template = html_template.replace("__MIN_LAT__", str(min_lat))
    html_template = html_template.replace("__MAX_LAT__", str(max_lat))
    html_template = html_template.replace("__MIN_LON__", str(min_lon))
    html_template = html_template.replace("__MAX_LON__", str(max_lon))
    
    html_template = html_template.replace("__GEOJSON_DATA__", geojson_data)
    html_template = html_template.replace("__BALAT_GEOJSON__", balat_geojson)
    html_template = html_template.replace("__PALET_JSON__", palet_json)
    html_template = html_template.replace("__YOLLAR_GEOJSON__", yollarData)
    
    html_template = html_template.replace("__LANDMARKS_GERCEK_JSON__", landmarks_gercek_json)
    html_template = html_template.replace("__LANDMARKS_ABSTRACT_JSON__", landmarks_abstract_json)
    html_template = html_template.replace("__MANUEL_PIN_JSON__", manuel_pin_json)
    html_template = html_template.replace("__MOST_DOMINANT_COLOR__", f"'{most_dominant_color}'")

    return html_template

# ==========================================
# 4. KONTROL PANELİ (SAYFAYI GÖSTER)
# ==========================================
...
with st.spinner('Kentsel veriler ve 1800 cephe hesaplanıyor, lütfen bekleyin...'):
    harita_html = haritayi_hazirla()
