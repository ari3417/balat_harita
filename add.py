...
KLASOR_SEMBOLIK_GERCEK = '/content/drive/MyDrive/balat_sembolik_binalar_gerçek_fotoğraflar/'
KLASOR_SEMBOLIK_ABSTRACT = '/content/drive/MyDrive/balat_sembolik_binalar_abstract/'
CIKTI_HTML = 'Balat_Interactive_Dashboard.html'
...
# ==========================================
# 4. KASMA ÖNLEYİCİ GÖRSEL VE RENK (DATA ART)
# ==========================================
print("[3/5] Kasmayı önlemek için görsellerin netliği düşürülüyor ve renkler çekiliyor...")
...
for idx, row in eslesen_veriler.iterrows():
    ...
    if str(kat_val).strip().lower() == 'nan': 
        kat_val = 'Belirtilmemiş'
    else: 
        kat_val = str(kat_val).replace('.0', '')
    ...
    if zemin_val == 'Belirtilmemiş':
        for col in row.index:
            if 'fonks' in str(col).lower() and pd.notna(row[col]) and str(row[col]).strip().lower() != 'nan':
                zemin_val = str(row[col]).strip()
                break
    
    ozellikler.append({
        "id": str(row['Bina_ID']),
        "img": b64_img,
        "malzeme": malzemeler[0] if malzemeler else "Sıvalı",
        "renk_orj": hex_renk,
        "sokak": sokak_ismi,
        "kat": kat_val,
        "zemin": zemin_val
...
for i in range(palet_25_model.n_clusters):
    ...
    if len(cluster_colors) == 0:
        best_c = palet_25_model.cluster_centers_[i]
    else:
        best_c = max(cluster_colors, key=get_vibrancy)
    palet_25_colors.append(rgb_to_hex(best_c))

labels_25_full = palet_25_model.predict(X)

counts = np.bincount(labels_25_full)
...
# ==========================================
# YENİ: KML DOSYASINDAN MANUEL PİNLERİ YÜKLEME
# ==========================================
print("KML dosyasından pin koordinatları alınıyor...")
manuel_pin_data = []
...
try:
            ...
            for pm in placemarks:
                ...
                if name_tag and coord_tag:
                    ...
                    if len(coords) >= 2:
                        ...
                        bulunan_b64 = ""
                        if os.path.exists(KLASOR_SEMBOLIK_ABSTRACT):
                            # KML'deki isim ile klasördeki dosyayı daha esnek eşleştiriyoruz
                            temiz_isim = isim.replace('_', ' ').lower()
                            for dosya_adi in os.listdir(KLASOR_SEMBOLIK_ABSTRACT):
                                e_temiz_isim = os.path.splitext(dosya_adi)[0].replace('_', ' ').lower()
                                if temiz_isim in e_temiz_isim or e_temiz_isim in temiz_isim:
                                    yol = os.path.join(KLASOR_SEMBOLIK_ABSTRACT, dosya_adi)
    ...
    print(f"KML okuma hatası veya kml dosyası yok: {e}")

manuel_pin_json = json.dumps(manuel_pin_data)

# ==========================================
# YENİ: SEMBOLİK BİNALARI YÜKLEME (INTRO İÇİN)
# ==========================================
print("Sembolik Binalar Intro için hazırlanıyor...")

def get_landmark_base64(image_path):
    ...
    if os.path.exists(folder_path):
        for img_name in os.listdir(folder_path):
            if img_name.endswith(('.png', '.jpg', '.jpeg')):
                ...
                b64 = get_landmark_base64(img_path)
                if b64: arr.append(b64)
    return arr

sembolik_gercek = load_folder_to_b64(KLASOR_SEMBOLIK_GERCEK)
sembolik_abstract = load_folder_to_b64(KLASOR_SEMBOLIK_ABSTRACT)

landmarks_gercek_json = json.dumps(sembolik_gercek)
landmarks_abstract_json = json.dumps(sembolik_abstract)

# ==========================================
# 5. BALAT SINIRI (MASKE)
# ==========================================
...
# ==========================================
# 6. MÜKEMMEL UI VE NATIVE ZOOM HEATMAP
# ==========================================
...
<head>
    ...
    <style>
        ...
        .selected-card {{
            display: flex; align-items: center; background: white; padding: 5px 12px; 
            border-radius: 12px; border: 1px solid #ddd; min-width: 200px; 
            box-shadow: 0 2px 5px rgba(0,0,0,0.05); flex-shrink: 0;
            transition: 0.2s; cursor: pointer; margin-right: 5px;
        }}
        ...
        #collage-overlay {{ 
            ...
            background: rgba(245, 245, 245, 0.98); z-index: 3000; 
        ...
        #collage-content {{
            position: absolute; top: 0; left: 0; width: 100%; height: 100%;
            transform-origin: 0 0; will-change: transform; transition: transform 0.1s ease-out;
        }}
        
        .flying-facade {{ 
            position: absolute; top: 0; left: 0; 
            transition: transform 1.2s cubic-bezier(0.25, 1, 0.5, 1), opacity 0.3s;
            object-fit: contain; filter: drop-shadow(2px 2px 4px rgba(0,0,0,0.3)); 
            z-index: 3001; will-change: transform; backface-visibility: hidden;
        }}

        /* =======================================================
           YENİ: GİRİŞ EKRANI (INTRO) TASARIMI 
           ======================================================= */
        #intro-screen {{
            position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
            background: #fcfcfc; 
            z-index: 9999; display: flex; align-items: center; justify-content: center;
            overflow: hidden; transition: background 1.5s ease; cursor: pointer;
        }}
        #intro-collage {{
        ...
        .intro-col {{
            ...
            transition: transform 1.5s cubic-bezier(0.25, 1, 0.5, 1), opacity 1s;
        }}
        .intro-col.intro-col-large {{
            flex: 4; 
            z-index: 5;
            filter: drop-shadow(5px 5px 15px rgba(0,0,0,0.3));
        }}
        .intro-col img {{ 
            width: 100%; height: auto; object-fit: cover; flex-grow: 1; 
        }}
...
<body>
                ...
                <div class="white-box">
                    <div class="mat-container">
                        <div class="mat-row" onclick="filterMaterial('Tuğla', this)">
                            <div class="mat-icon-wrapper"><div class="icon-tugla-bg"></div></div>
                            <span class="mat-text">brick (tuğla)</span>
                        </div>
                        <div class="mat-row" onclick="filterMaterial('Ahşap', this)">
                            <div class="mat-icon-wrapper"><div class="icon-ahsap-bg"></div></div>
                            <span class="mat-text">wood (ahşap)</span>
                        </div>
                        <div class="mat-row" onclick="filterMaterial('Sıvalı', this)">
                            <div class="mat-icon-wrapper"><div class="icon-sivali-bg"></div></div>
                            <span class="mat-text">plaster (sıvalı)</span>
                        </div>
                    </div>
                    
                    <h3 class="box-title title-blue">MATERIAL</h3>
                    <p class="desc">Choose the buildings<br>that have the <b>material</b><br>you want to see.</p>
                </div>
                
                <div class="white-box">
                    <div class="color-graphic" id="color-graphic"></div>
                    
                    <h3 class="box-title title-orange">COLOR</h3>
                    <p class="desc">Choose the buildings in<br>the <b>color</b> you want to<br>see.</p>
                </div>
                
                <div class="dominant-color-container" onclick="filterColor('{most_dominant_color}')">
                    <div class="dominant-circle"></div>
                    <div class="dominant-text">the most<br>dominant color</div>
                </div>
    ...
    <script>
        ...
        map.createPane('yollarPane');
        map.getPane('yollarPane').style.zIndex = 700;
        map.getPane('yollarPane').style.pointerEvents = 'none';

        map.createPane('sinirPane');
        map.getPane('sinirPane').style.zIndex = 800; 
        map.getPane('sinirPane').style.pointerEvents = 'none';
        ...
        // YENİ: KML DOSYASINDAN GELEN MANUEL PİNLER
        ...
        L.geoJSON(balatSiniri, {{ 
            pane: 'sinirPane',
            style: {{ color: '#ea1f48', weight: 2, fillOpacity: 0, dashArray: '5, 5' }} 
        }}).addTo(map);

        L.geoJSON(yollarData, {{
            pane: 'yollarPane',
            style: {{ color: '#ffffff', weight: 3, opacity: 1.0 }},
        ...
        let activeMaterial = 'Tümü';
        let activeColor = 'Tümü';
        let semanticStep = 0;
        let isBottomBarOpen = true;
        
        let selectedFeatures = [];

        window.removeBuilding = function(id) {{
        ...
        window.selectBuilding = function(props) {{
            if (!selectedFeatures.some(f => f.id === props.id)) {{
                selectedFeatures.unshift(props);
            }}
            renderBottomBar();
            if(!isBottomBarOpen) toggleBottomBar();
            
            setTimeout(() => {{
                document.getElementById('bottombar').scrollLeft = 0;
            }}, 50);
        }};

        window.selectBuildingFromIcon = function(el, id, img, sokak, kat, zemin, malzeme) {{
            selectBuilding({{ id: id, img: img, sokak: sokak, kat: kat, zemin: zemin, malzeme: malzeme }});
        }};
        ...
        function renderBottomBar() {{
            ...
            selectedFeatures.forEach(props => {{
                const safeSokak = (props.sokak || "Bilinmiyor").replace(/'/g, "");
                const safeKat = (props.kat || "").replace(/'/g, "");
                const safeZemin = (props.zemin || "").replace(/'/g, "");
                const safeMalzeme = (props.malzeme || "").replace(/'/g, "");
                
                const cardHtml = `
                    <div class="selected-card" onclick="removeBuilding('${{props.id}}')">
                        <img src="${{props.img}}">
                        <div class="selected-card-info">
                            <span><b>Street:</b> ${{safeSokak}}</span>
                            <span><b>Floors:</b> ${{safeKat}}</span>
                            <span><b>Ground:</b> ${{safeZemin}}</span>
                            <span><b>Material:</b> ${{safeMalzeme}}</span>
                        </div>
                    </div>
                `;
            ...
            if ((activeMaterial !== 'Tümü' || activeColor !== 'Tümü') && !isBottomBarOpen && (visibleCount > 0 || selectedFeatures.length > 0)) {{
                toggleBottomBar();
            }}
        }}

        let poligonLayer = L.geoJSON(geojsonData, {{
            pane: 'heatmapPane',
            style: function(feature) {{ return {{ fillColor: 'transparent', color: 'transparent' }}; }}
        }}).addTo(map);
        ...
        map.getContainer().addEventListener('wheel', function(e) {{
            ...
            if (semanticStep > 0) {{
                ...
                if (e.deltaY > 0) {{ 
                    if (semanticStep < 5) {{ semanticStep++; updateMap(); }}
                }} else if (e.deltaY < 0) {{ 
                    semanticStep--; updateMap();
                    if (semanticStep === 0) map.scrollWheelZoom.enable();
                }}
                return; 
            }}
            ...
            if (semanticStep === 0 && !map.scrollWheelZoom.enabled()) {{
        ...
        function updateMap() {{
            ...
            let status = "";
            if (z > 16) {{
                status = "Architectural Abstracted Facades";
            }} else {{
            ...
            poligonLayer.setStyle(function(feature) {{
                ...
                let fillColor = props.renk_orj;
                let strokeColor = '#ffffff'; 
                let fillOp = isHidden ? 0.05 : 0.95;
                let strokeOp = 0;
                let weight = 0;
                
                if (z > 16 || semanticStep === 0) {{ 
                    return {{ fillColor: fillColor, fillOpacity: 0.0, color: 'transparent', weight: 0 }}; 
                }}
                ...
                if (semanticStep >= 1) {{
                    ...
                    weight = semanticStep * 15; 
                    
                    fillOp = isHidden ? 0.0 : 0.85; 
                    strokeOp = isHidden ? 0.0 : 0.4; 
                }}
            ...
            geojsonData.features.forEach(feature => {{
                ...
                if ((z > 16 || semanticStep === 0) && !isHidden) {{
                    const iconHtml = `<div style="width:100%; height:100%; display:flex; justify-content:center; align-items:center; transition:0.2s;">
                                        <img src="${{props.img}}" style="max-height:100%; max-width:100%; object-fit:contain; filter: drop-shadow(1px 1px 1px rgba(0,0,0,0.5)); transition:0.2s;">
                                      </div>`;
                    const icon = L.divIcon({{ html: iconHtml, className: 'custom-facade-icon', iconSize: [12, 24], iconAnchor: [6, 12] }});
                    let marker = L.marker([props.center_lat, props.center_lon], {{ icon: icon }}).addTo(iconLayer);
                }}
            }});
            
            // YENİ DÜZELTME: MANUEL PİNLERİN BOYUTU STANDART BİNALARLA BİREBİR AYNI YAPILDI [12, 24]
            if (z > 16 || semanticStep === 0) {{
                manuelPinsData.forEach(pin => {{
                    if (pin.b64 && pin.b64 !== "") {{
                        const iconHtml = `<div style="width:100%; height:100%; display:flex; justify-content:center; align-items:center; transition:0.2s;">
                                            <img src="${{pin.b64}}" style="max-height:100%; max-width:100%; object-fit:contain; filter: drop-shadow(1px 1px 2px rgba(0,0,0,0.8)); transition:0.2s;">
                                          </div>`;
                        // Boyut standartlaştırıldı: [12, 24] ve Çapa noktası [6, 12]
        ...
        const cContent = document.getElementById('collage-content');

        overlay.addEventListener('wheel', (e) => {{
            if (!isCollageMode) return;
            e.preventDefault();
            
            const zoomPointX = e.clientX;
            const zoomPointY = e.clientY;

            let zoomDelta = e.deltaY < 0 ? 1.15 : 0.85;
            let newZoom = collageZoom * zoomDelta;
            
            if (newZoom < 1.0) newZoom = 1.0; 
            if (newZoom > 15) newZoom = 15;   
            
            collagePanX = zoomPointX - (zoomPointX - collagePanX) * (newZoom / collageZoom);
            collagePanY = zoomPointY - (zoomPointY - collagePanY) * (newZoom / collageZoom);
            
            collageZoom = newZoom;
            cContent.style.transition = 'none'; 
        ...
        overlay.addEventListener('mousedown', (e) => {{
            ...
            startDragX = e.clientX - collagePanX;
            startDragY = e.clientY - collagePanY;
            overlay.style.cursor = 'grabbing';
        }});

        window.addEventListener('mousemove', (e) => {{
            if (!isDraggingCollage) return;
            collagePanX = e.clientX - startDragX;
            collagePanY = e.clientY - startDragY;
            cContent.style.transition = 'none';
            cContent.style.transform = `translate3d(${{collagePanX}}px, ${{collagePanY}}px, 0) scale(${{collageZoom}})`;
        }});
        ...
        function toggleCollage() {{
            const btnText = document.querySelector('.facade-btn-text');
            const btnIcon = document.querySelector('.facade-btn-icon');
            ...
            if (isCollageMode) {{
                ...
                let baseHeight = Math.floor(totalBuildings / cols);
                let remainder = totalBuildings % cols;
                
                let colCapacities = new Array(cols).fill(baseHeight);
                
                for(let k=0; k<remainder; k++) {{
                    let mid = Math.floor(cols/2);
                    let offset = k % 2 === 0 ? k/2 : -Math.ceil(k/2);
                    colCapacities[(mid + offset + cols) % cols]++;
                }}
                
                for (let k = 0; k < Math.floor(totalBuildings * 0.1); k++) {{
                    let from = Math.floor(Math.random() * cols);
                    let to = Math.floor(Math.random() * cols);
                    if (colCapacities[from] > 1) {{
                        colCapacities[from]--;
                        colCapacities[to]++;
                    }}
                }}
                ...
                for (let c = 0; c < cols; c++) {{
                    ...
                    for (let r = 0; r < capacity; r++) {{
                        ...
                        img.src = f.properties.img;
                        img.className = 'flying-facade';
                        
                        let startTransformX = pt.x - collagePanX;
                        let startTransformY = pt.y - collagePanY;
                        let startScale = 12 / imgW; 
                ...
                flyingElements.forEach((item, i) => {{
                    setTimeout(() => {{
                        item.el.style.transform = `translate3d(${{item.trgX}}px, ${{item.trgY}}px, 0) scale(1)`;
                    }}, 50 + (i * 2));
                }});
                
            }} else {{
                btnText.innerText = "click for all facade";
                btnIcon.innerText = "▶";
                ...
                collagePanY = 0;
                cContent.style.transition = 'transform 1.2s cubic-bezier(0.25, 1, 0.5, 1)';
                cContent.style.transform = `translate3d(0px, 0px, 0) scale(1)`;
                ...
                flyingElements.forEach((item, i) => {{
                    ...
                    setTimeout(() => {{
                        item.el.style.transform = `translate3d(${{pt.x}}px, ${{pt.y}}px, 0) scale(${{item.tS}})`;
                ...
                setTimeout(() => {{
                    ...
                    flyingElements.forEach(item => item.el.remove());
                    flyingElements = [];
        ...
        // =========================================================
        // YENİ: OTOMATİK AKILLI GİRİŞ (INTRO) EKRANI MANTIĞI
        // =========================================================
        function initIntro() {{
            let features = [...geojsonData.features];
            let allImages = [];
            
            // Tüm binaları havuza ekle
            features.forEach(f => {{
                allImages.push(f.properties.img);
                
                let s = (f.properties.sokak || "").toLowerCase();
                if (s.includes('kiremit') || s.includes('merdiven')) {{
                    allImages.push(f.properties.img);
                    allImages.push(f.properties.img);
                }}
            }});
            
            // Gerçek sembolik binaların havuzda kaybolmaması için 40 KAT ağırlık veriyoruz
            for(let i=0; i<40; i++) {{
                if(specialLandmarksGercek && specialLandmarksGercek.length > 0) {{
                    allImages = allImages.concat(specialLandmarksGercek);
                }}
            }}
            ...
            const collage = document.getElementById('intro-collage');
            let numCols = 30; // Fotoğraftaki gibi ince çoklu şeritler
            
            // Abstract sembolik binaları daha büyük yapmak için ayrı bir havuz
            let abstractLandmarks = [];
            if (specialLandmarksAbstract && specialLandmarksAbstract.length > 0) {{
                // Yeterli çeşitlilik için abstract havuzunu genişletip karıştır
                for(let i=0; i<10; i++) abstractLandmarks = abstractLandmarks.concat(specialLandmarksAbstract);
                abstractLandmarks.sort(() => Math.random() - 0.5);
            }}
            ...
            for(let i=0; i<numCols; i++) {{
                ...
                // Abstract binaları belirli kolonlara daha büyük şekilde yerleştir
                if ((i === 4 || i === 10 || i === 16 || i === 22 || i === 28) && abstractLandmarks.length > 0) {{
                    col.classList.add('intro-col-large');
                    
                    let imgCount = Math.floor(Math.random() * 2) + 3; // Daha az ama büyük
                    for(let j=0; j<imgCount; j++) {{
                        let img = document.createElement('img');
                        let absImg = abstractLandmarks.shift() || abstractLandmarks[0];
                        img.src = absImg;
                        col.appendChild(img);
                    }}
                }} else {{
                ...
                col.style.transform = `translateY(${{offset}}%)`;
        ...
        function closeIntro() {{
            ...
            const cols = document.querySelectorAll('.intro-col');
            const centerContent = document.querySelector('.intro-center-content');
            
            // Önce ortadaki metin kutusunu erit
            centerContent.style.opacity = '0';
            centerContent.style.transform = 'scale(0.8)';
            
            // Sonra kolonları sırayla yukarı ve aşağı doğru fırlat
            cols.forEach((col, i) => {{
                let dir = i % 2 === 0 ? -100 : 100; // Çiftler yukarı, tekler aşağı uçar
                col.style.transform = `translateY(${{dir}}vh)`;
                col.style.opacity = '0';
            }});
            
            intro.style.background = 'transparent';
            
            // Animasyon bittikten sonra intro ekranını tamamen kapat
            setTimeout(() => {{
                intro.style.visibility = 'hidden';
                updateMap(); // Haritayı kusursuz pozisyonunda (Aşama 0) başlat
            }}, 1500);
        }}

        // Site açılışında intro ekranını inşa et ve haritayı arkada hazırla
        setTimeout(() => {{
            initIntro();
            updateMap(); // Harita Aşama 0 pozisyonunda hazır bekler
        }}, 100);
